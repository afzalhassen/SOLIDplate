...
namespace SOLIDplate.Common.Portable
{
    public static class QueryableExtensions
    {
        public static IQueryable<T> OrderBy<T>(this IQueryable<T> source, string sortExpression)
        {
            ...
            var orderByMethodNameToUse = (isDescending ? $"{orderByMethodName}Descending" : orderByMethodName);
            var propertyName = sortExpressionParts[0];
            var propertyInfo = typeOfT.GetRuntimeProperty(propertyName);

            if (propertyInfo == null)
            {
                var message = $"No property named '{propertyName}' exists on the type '{typeOfT.Name}'.";
